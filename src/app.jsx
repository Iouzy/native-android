...
function UpdateChecker({ accentColor, store }) {
  ...
  // When the system installer closes (success or failure) and the user returns
  // to the app, show the conflict guidance again by resetting to "available".
  // The app has no way to know if the install succeeded — the user can verify
  // by tapping "Check updates"; if it worked, the new version is already
  // running and it will show "Up to date".
  useEffect(() => {
    if (state.kind !== "installing") return;
    const onVisible = () => {
      if (!document.hidden) {
        setState(s => s.kind === "installing" ? { kind: "available", url: s.url } : s);
      }
    };
...
function App() {
  ...
  // One-time cleanup of a stale "we already asked for notifications" flag. Every
  // build before the native bridge was fixed could set `pauta.notifAsked` WITHOUT
  // ever actually reaching the OS permission request — the bridge silently
  // no-opped (registerPlugin didn't exist), so the system dialog never fired. That
  // false-positive flag makes FocusNotifControl show "open system settings"
  // instead of the real "allow notifications" prompt. Clear it once (guarded by a
  // versioned marker) so the proper flow — which now genuinely fires the OS
  // dialog — can run again for anyone upgrading from a broken build.
  useEffect(() => {
    try {
      if (!localStorage.getItem("pauta.notifAsked.cleared.v1")) {
        localStorage.removeItem("pauta.notifAsked");
        localStorage.setItem("pauta.notifAsked.cleared.v1", "1");
      }
    } catch (_) {}
  }, []);
