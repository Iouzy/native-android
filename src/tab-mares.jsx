...
function NewHabitForm({ accentColor, onSubmit, onCancel }) {
  ...
  const submit = () => {
    if (!name.trim()) return;
    // Numeric fields may be momentarily blank while editing (so they can be
    // cleared and retyped on mobile); coerce to a safe value here.
    const days = Math.max(1, parseInt(periodDays, 10) || 1);
    const mday = Math.min(31, Math.max(1, parseInt(monthday, 10) || 1));
    let endsAt = null;
    if (recurrence === "period") {
      endsAt = Date.now() + (days - 1) * 86400000;
    }
    let anchor = null;
    if (dayMode === "fixed") {
      if (cadence === "weekly") anchor = weekday;
      else if (cadence === "monthly") anchor = mday;
    }
              ...
              {cadence === "monthly" && dayMode === "fixed" && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  ...
                  <input type="number" min="1" max="31" value={monthday}
                    onChange={e => { const v = e.target.value; setMonthday(v === "" ? "" : Math.min(31, Math.max(1, parseInt(v, 10) || 1))); }}
                    onBlur={() => setMonthday(m => Math.min(31, Math.max(1, parseInt(m, 10) || 1)))}
                    style={{
          ...
          {countable && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <input type="number" min="2" max="99" value={target}
                onChange={e => { const v = e.target.value; setTarget(v === "" ? "" : Math.min(99, Math.max(1, parseInt(v, 10) || 1))); }}
                onBlur={() => setTarget(t => Math.max(2, parseInt(t, 10) || 2))}
          ...
          {recurrence === "period" && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              ...
              <input type="number" min="1" max="365" value={periodDays}
                onChange={e => { const v = e.target.value; setPeriodDays(v === "" ? "" : Math.min(365, Math.max(1, parseInt(v, 10) || 1))); }}
                onBlur={() => setPeriodDays(d => Math.min(365, Math.max(1, parseInt(d, 10) || 1)))}
                style={{
