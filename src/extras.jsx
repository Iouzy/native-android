...
// When the user JUST turns reminders on, mark every reminder whose time has
// already passed today as already-fired — otherwise the in-app loop below would
// dump the whole day's past-due nudges (plano do dia, hábitos, reflexão, e os
// relógios por hábito) all at once the instant notifications are granted. Future
// reminders are untouched, so they still fire at their scheduled minute. (The
// native AlarmManager already schedules past times for tomorrow, so this only
// needs to tame the in-app loop.)
function seedPastDueReminderFlags(rem, state) {
  try {
    const dayKey = dayKeyOf(Date.now());
    const now = new Date();
    const hhmm = pad(now.getHours()) + ":" + pad(now.getMinutes());
    const mark = (kind) => localStorage.setItem("pauta.reminded." + kind + "." + dayKey, "1");
    if (rem.plannerTime && hhmm >= rem.plannerTime) mark("planner");
    if (rem.habitsTime && hhmm >= rem.habitsTime) mark("habits");
    if (rem.reflectionTime && hhmm >= rem.reflectionTime) mark("reflection");
    for (const h of (state.habits || [])) {
      if (h.clock && hhmm >= h.clock) mark("clock_" + h.id);
    }
  } catch (_) {}
}
