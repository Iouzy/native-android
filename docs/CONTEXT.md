...
| `docs/FIELD_FIXES.md` | Defects found by *using* the app. Ordered by what each costs the person using it | F1…F13 | F1…F7 **done** (F7 partial — its chrome half waits on L4); F8 **done**; F9 **done** (partial); F10…F13 pending |
