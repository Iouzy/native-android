...
### P4 · Surface primitives + radius scale — Status: done (PR #145)
...
### P7 · Pauta sweep — Status: done (PR #148)
...
### P9 · Sheets & Settings sweep — Status: done (PR #151)
...
### P10 · Micro-interactions, empty states, haptic map — Status: done (PR #152)
...
2026-07-31 · P7 · #148 · PautaType.Timer (tnum, 42sp) on the active card + zen's 64sp `.copy()`; TargetUnderline hairline under the digits toward targetMs (un-animated — rides the existing 1s tick, so no reduced-motion branch and no extra frames; the F1 tide stays the mood, the fio is the number) and mirrored onto the book session card, which had no target feedback at all; hero slot start↔active now an AnimatedContent crossfade (fadeIn Base / fadeOut Fast, SizeTransform Base, contentKey=block id so the tick never restarts it; EnterTransition.None + snap size under reducedMotion) in both faces; three dark-surface duplicates collapsed to shared internal atoms — DarkPillButton (was + BookSession's byte-identical DarkOutlinePill), DarkAccentPill, ResumePill; eyebrows on dark surfaces (start CTA, "em curso", zen, meta-cumprida prompt, history drill-down) → SectionEyebrow(onDark2/accent); block-title lines unified to CardTitle across Start/Pause/Conclude/Switch/Edit sheets + zen + start CTA (was 19/20/22/24sp); P5's last 9sp leftovers (HeaderChip, "hoje" tag) and the mono meta lines in timeline/paused/history/edit rows → Meta/MetaSmall; paused-row titles → Label (their cards lose ~6dp of inherited 24sp line-height — deliberate, it matches Hoje's rows exactly). Leftovers: timeline row titles stay 15sp (Label at 14 would flatten them against the meta), PStat's 8sp strip labels left alone (10sp would rewrap the 3-up), zen gets no underline (the full-screen tide already is its progress), history day rows keep their own backgrounds so PautaCard's fixed paper2 doesn't drop the today tint — P9 if PautaCard ever takes a fill
2026-07-31 · P8 · #149 · perf: `modelsById` built once per data change (was 3 `modelOf(h)` per habit per pass), `visibleHabits`/`models`/`overall` memoised, and every row's month+streak math collapsed into one `remember`-ed `RowStats` keyed on (model, year, month, today, isCurrentMonth) — HabitModel's data-class equality is what lets untouched rows keep their memo when a sibling is marked. Method (static call-count audit, no device in this env): per composition pass with V visible tides the screen ran ~9 HabitCalculator history walks per tide (3 for the overall %, 6 per row incl. `bestStreak` walking creation→today); a pure recomposition now runs 0, and marking a day runs 3V + 6 instead of 9V (V=10 → 36 vs 90). Bug found in passing: `days` was keyed on the model, which doesn't carry `target`, so adding a target to an existing tide left every cell state stale — `isCount` joins the keys. Visual: month navigation slides in the direction travelled (`MonthSlide`, an AnimatedContent over `year*12+month-1` so the direction is free and multi-month jumps still read right; Base in/Fast out, EnterTransition.None + snap size under reducedMotion) on both the PeriodLabel and the eyebrow+headline block; P5's Marés leftovers closed — overall-% → `CardTitle.copy(30sp)`, "marés passadas" link + hint card + cadence/count chips + maturity + tier badge + streak + best-streak → MetaSmall, "legenda" → SectionEyebrow, empty-state explanation + hidden-tides note → Body italic, "adicionar maré" → Label (the dashed button loses ~6dp of inherited 24sp line-height, same deliberate trade as P7's paused rows), remove-confirmation → CardTitle; book face: annual-goal card already on PautaCard from P4, so it only took CardTitle/MetaSmall. Deliberate leftovers: the row's 16sp habit name and 13sp italic time (no role fits; matches Hoje's TodayTideRow, still P10's call), the tier badge keeps SemiBold + 0.54sp tracking rather than SectionEyebrow's 1.6sp (a row badge, not a section header — 1.6 would push the right column into the headline), the 8sp PARTIAL cell count and the legend popup's 11sp footnote left literal, and the Marés headline still sits below its month strip (P5 flagged the restructure as a layout shift; it's outside P8's spec).
2026-07-31 · P9 · #151 · sheets: PautaSheet now owns the four structural gaps (SheetGutter 24 / SheetFieldGap 18 / SheetLabelGap 8 / SheetActionGap 22), swept through all five sheet files — the 6/10/14/16/18/20/22/24 mix is gone; gaps tighter than the scale (title→field, serif hint under a group) stay local literals. One entrance: 0→1 over Slow on the house easing. M3 owns the bottom sheet's slide and exposes no spec to retune, so the piece both faces *share* is the content fade; the centred card, which M3 gives no motion at all, adds a 10dp rise so the two read as one gesture — under reducedMotion the value starts at 1 (no flash), and the fade modifier drops out once settled so a resting sheet carries no extra layer. One header: the centred card's title was a private mono 10sp/1.8 copy at a 22dp gutter → SheetEyebrow at 24dp, so the two faces now differ only in the dismiss affordance (handle vs. ×); SheetEyebrow gained a `modifier` param for the title row's weight(1f). Spot fixes: BookDetailSheet's roles (book + finish titles → CardTitle, author/progress/empty → Meta, series/format/pace/session/page → MetaSmall, note bodies → Body, the note-kind tag — an inline eyebrow at mono 9sp/0.9 — → SectionEyebrow), QuoteCaptureSheet (book titles → Body, "p." → Meta), radii tokenised (recurrence options, book picker → Field). Settings: headers → SectionEyebrow (was private sans 11sp SemiBold), SectionCard → PautaCard (the file's last hand-rolled clip+border+paper2), one row height everywhere (11dp vertical + a 56dp floor, so a text row lines up with the 32dp M3 Switch beside it rather than sitting shorter), and a real break before the danger zone (extra gap + full-width rule + header in the danger red its rows already use). Leftovers: the other PautaSheet callers (Insights/Trend/Routines/HabitDetail/WeekAhead/DateTimePickers) inherit the shell anatomy + entrance for free but their internal gaps are untouched — they weren't in P9's file list; BookDetailSheet's session reflection stays 13sp italic (Body at 15 would outweigh the row above it) and the format tag keeps its 0.54sp tracking (a badge, not a section header); the ad-hoc empty lines are P10's EmptyState.
2026-07-31 · P10 · #152 · three primitives in `ui/Modifiers.kt` + `ui/EmptyState.kt`, all four additive (with haptics off and motion reduced every one drops out and nothing behaves differently). `pressScale(enabled)`: a 0.97 dip on the house spring, self-contained — it *observes* the pointer stream (`awaitFirstDown(requireUnconsumed = false)` + `waitForUpOrCancellation()`, nothing consumed) so the `clickableNoRipple` beside it still owns the click, and a gesture taken over by a parent LazyColumn releases the dip instead of leaving the card stuck small; applied to `PautaCard(onClick)`, `PautaButton(enabled)`, the tab items and `SelectPill` (static cards and disabled buttons read no pref and get no layer). `EmptyState(line, title?, pip)`: the app had two treatments — the tabs' serif phrase with a Pip pose and the book faces' mono one-liner — now one serif-italic `ink4` line with an optional `ink2` CardTitle lead; six sites (Hoje no-intentions, Pauta timeline, Marés, shelf "a ler agora", session history, BookDetail notes + sessions), and Marés is the one needing both slots (intro phrase → title, explanation → line, keeps its 44dp Pip). `HapticFeedback.tick(prefs)` is now the app's only haptic call: tab settle and F1's target-crest routed through it, plus new ticks on intention check, block start/conclude (both faces), tide day-fill/count/respiro and the two-step delete arms in BookDetailSheet — pause/resume stay silent (mid-flight, not a commitment). Entrance stagger: `rememberEntrancePlay(id, enabled)` + `Modifier.entranceStagger(index, play)` — fade + 12dp rise over `Fast`, 30ms steps capped at 8, on Hoje's intentions (counted across the time-of-day groups so it reads as one sequence), Pauta's timeline and Marés' tides. Two guards make it "per session, not per scroll": a process-level `entrancesPlayed` set (not a `remember` — Settings and back must not replay it) and a 500ms armed window, after which the modifier leaves the chain, so items composed later *by scrolling* arrive plainly. Note P1 keeps all three tabs composed, so "first composition" is app start for all of them: the stagger is seen on the tab the app opens into and swiping to the others is left alone — deliberate, since animating a list during a swipe is exactly the hitch P1 removed. Deliberate leftovers: `EmptyState` has no `icon` slot (no call site wanted one — Pip is the app's icon); the ad-hoc empty lines outside the spec's six (HistoryView, InsightsSheet, RoutinesSheet, GoalsScreen, QuoteCaptureSheet, WeekAheadSheet) are untouched; `PautaCard(onClick)` now reads the motion pref per card, i.e. one prefs collector per tappable card — fine at these counts but worth a CompositionLocal if a future screen renders dozens.
