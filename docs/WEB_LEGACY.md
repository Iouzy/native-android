# Web app (legacy) — reference only

> The web/PWA build under `src/`, `index.html`, `vendor/`, `scripts/`, `tools/`
> is the **original** Pauta and the shipping release until the native cutover.
> Active development has moved to the native APK (`app-native/`). This file
> holds the web-app guidance that used to live in `CLAUDE.md`, kept out of the
> always-loaded root file to save context. Read it only when a task genuinely
...
The web build's native Android layer is Kotlin in `native/android/`: a
foreground-service focus-timer plugin (`FocusActivityPlugin`, `FocusService`,
...
> Note: this `native/android/` Capacitor layer is the **web build's** native
> shell — distinct from the from-scratch Kotlin/Compose rewrite under
> `app-native/`, which is where active work happens.
...
> `android.yml` still runs on **every** push (including `app-native/`-only
> branches), so it must stay green even when you're working on the native APK.
> Native-only changes don't touch `src/`, so `npm run check` keeps passing
> untouched — that job going red on a native PR means something unrelated broke.
...
## Workflow — how to ship web changes
...
2. **Run `npm run check`** before committing — it catches JSX syntax errors,
   missing i18n keys, cadence math bugs, and Vitest failures.
3. **Commit**, **push**, and **open a PR** targeting `main`.
4. **Subscribe** to PR activity and **wait for CI**. If CI fails, diagnose and
   fix on the same branch (never a new PR for a CI fix).
5. When green, **merge to `main`** using squash.
6. After merging, **verify** the release published (`get_latest_release`: the
   tag moved to the new commit and the asset is `pauta-v<N>.apk`).
</content>
