...
### R4 · EPUB reader — Status: done (PR #172)
...
2026-08-02 · R4 · #172 · o último por fazer, e o único que faltava para a app ler o livro que o dono lhe deu. A decisão que arrasta tudo o resto: **não há parser de XML**. O contentor, o OPF e os capítulos passam por um scanner escrito à mão que percebe etiquetas e mais nada — não vai buscar uma entidade externa porque não vai buscar coisa nenhuma, e não expande um *billion laughs* porque as únicas entidades que conhece são uma tabela fixa a que um documento não pode acrescentar-se. O §4 pede DTDs e expansão desligadas; não ter um parser que saiba o que é um DTD torna isso propriedade em vez de definição. É um ciclo e não uma gramática, por isso dez mil `<div>` aninhados custam dez mil iterações e nenhuma pilha. Nada é extraído para o disco: as entradas lêem-se por nome do arquivo aberto, e um href absoluto é recusado em vez de reescrito (era um bug meu, apanhado pelo teste que dizia que devia ser nulo). O higienizador é lista de permitidos porque as etiquetas de que um livro precisa são poucas e as que o HTML terá para o ano não são conhecíveis. No motor, a correcção menos óbvia foi o interceptor: devolver vazio a *tudo* incluía o próprio documento e o leitor teria ficado em branco — passa só o quadro principal, e o único que este WebView carrega é o nosso. A parte visível decidiu-se sozinha a partir do R6: um EPUB conta em **percentagem** em todo o lado porque não tem páginas (o texto reflui, e "página 80" seria um sítio diferente em cada tamanho de letra), e essa é exactamente a unidade em que o `wordsPerUnit` já pensava — a sessão, o ritmo e o WPM continuaram a funcionar sem uma linha de excepção, e um livro contado larga finalmente o `≈`. Ficou um compromisso honesto: a CSP não deixa passar tipos de letra, por isso o corpo é a `serif` do sistema e não a Instrument Serif, que é de títulos e má para ler ao metro.
