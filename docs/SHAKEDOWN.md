...
S2, S3 and S6 are all independent of each other — any order works. All three
have shipped, so what is left is **S5**, plus **S4** whenever the owner answers.

**Blocked on a decision:** S4, on the three questions in its table. S3's
existence was conditional on S1 and is now settled: it was real, and it is
fixed.
